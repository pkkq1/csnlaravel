...
//        }
        ...
        if($artistIds) {
            foreach ($artistIds as $key => $item) {
                if($item == -1) {
                    unset($artistIds[$key]);
                    $artistCharSet = trim(htmlspecialchars(Helpers::strReplaceSolr(trim(Helpers::khongdau($artistName[$key]))), ENT_QUOTES));
                    if($artistCharSet)
        ...
        //dd(htmlspecialchars(trim(Helpers::strReplaceSolr(trim(Helpers::khongdau($artistName[$key]))), ENT_QUOTES)));
