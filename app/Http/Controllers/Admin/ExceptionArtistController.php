...
use App\Models\ArtistExceptionModel;
use App\Solr\Solarium;
use App\Http\Controllers\Sync\SolrSyncController;

class ExceptionArtistController extends CrudController
{
    ...
    public function setup() {
        $this->crud->setModel("App\Models\ArtistExceptionModel");
        $this->crud->setEntityNameStrings('Ca sĩ', 'Ca Sĩ');
        $this->crud->setRoute(config('backpack.base.route_prefix').'/artist_exception');
...
//        ]);
    ...
    public function edit($id, $template = false)
    {
        ...
        return view('vendor.backpack.artist.artist_exception_edit', $this->data);
    }
...
//        $this->Solr->solrDeleteById('artist_'.$id);
    ...
    public function setExptionArtist(Request $request, $id) {
        $artistExist = $this->artistRepository->getModel()::where('artist_id', $id)->first();
        if(!$artistExist) {
...
//        $artistExist->delete();
    ...
    public function RemoveExptionArtist(Request $request, $id) {
        $artistExist = ArtistExceptionModel::where('artist_id', $id)->first();
        if(!$artistExist) {
            \Alert::error('Ca sĩ không đã tồn tại.')->flash();
            return redirect()->back();
        }
