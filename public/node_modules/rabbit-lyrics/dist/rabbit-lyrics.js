...
class RabbitLyrics {
     ...
     */
    parseLyrics() {
        let lyrics = this.element.textContent.trim().split('\n');
        lyrics = lyrics.replaceArray(array('[t1]', '[t2]', '[t3]', '[t4]', '[t5]'), '<br/>');
        console.log(lyrics);
        let lyricRaw = [];
...
;            if (timeStamps.length && lineElementsWithoutEndingTime.length) {
    ...
    decodeTimeStamp(timeStamp) {
        ...
        let results = timeStamp.match(/\[(\d+):(\d+\.\d+)\]/) || timeStamp.match(/\[(\d+):(\d+)\]/) || [];
