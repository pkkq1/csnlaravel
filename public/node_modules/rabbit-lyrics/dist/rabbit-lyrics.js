    ...
    /***/ (function(module, __webpack_exports__, __webpack_require__) {
        ...
        class RabbitLyrics {
            ...
            constructor(options) {
                ...
                this.jw_player = options.jw_player;
                ...
                this.lineElements = [];
                if(options.onUpdateTimeJw) {
                    this.element.scrollTop = 0;
                    this.lineElements = listLyrics;
                    this.synchronizer();
                }else{
             ...
             */
            parseLyrics() {
                // let lyrics = this.element.textContent.replaceArray(['\n[t1]', '\n[t2]', '\n[t3]', '\n[t4]', '\n[t5]'], '<br/>');
                let lyrics = this.element.textContent.replace(/(?:\n+\[t\d+])/g, '<br>');
                lyrics = lyrics.trim().split('\n');
                let lyricRaw = [];
                for (let i = 0; i < lyrics.length; i++) {
                    let line = lyrics[i].trim();
                    let timeStamps = line.match(/\[\d+:\d+\.\d+\]/g) || line.match(/\[\d+:\d+\]/g) || [];
                    ...
                    if(timeLength ==1) {
                        ...
                        for (let i = 0; i < timeLength; i++) {
                            let content = line.replace(/\[\d+:\d+\.\d+\]/g, '');
                            content = content.replace(/\[\d+:\d+\]/g, '');
                            lyricRaw.push(timeStamps[i] + content);
                        }
                    }
                }
                let lines = lyricRaw.sort();
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    let contentLyric = line.replace(/\[\d+:\d+\.\d+\]/g, '');
                    contentLyric = contentLyric.replace(/\[\d+:\d+\]/g, '');

                    // Look up time stamps
                    let timeStamps = line.match(/\[\d+:\d+\.\d+\]/g) || line.match(/\[\d+:\d+\]/g) || [];
                    let beginningTimeStamp = line.match(/^\[\d+:\d+\.\d+\]/g) || line.match(/^\[\d+:\d+\]/g) || [];
                    let endingTimeStamp = line.match(/\[\d+:\d+\.\d+\]$/g) || line.match(/\[\d+:\d+\]$/g) || [];
                    ...
                    let firstDecodeTime = this.decodeTimeStamp(timeStamps[0]);
                    if (timeStamps.length && lineElementsWithoutEndingTime.length) {
                        lineElementsWithoutEndingTime.forEach(function (element) {
                            element.dataset.end = firstDecodeTime;
                            endBefore = firstDecodeTime;
                        }, this);
                        lineElementsWithoutEndingTime = [];
                    }
                    if(contentLyric) {
                        let posTwoSub = contentLyric.indexOf("<br>");
                        let lineElement = document.createElement('div');
                        let karaElement = document.createElement('div');
                        lineElement.className = 'line';
                        karaElement.className = 'kara-csn';
                        karaElement.style.width = "0%";
                        this.element.appendChild(lineElement);
                        this.lineElements.push(lineElement);
                        listLyrics.push(lineElement);
                        ...
                        if (beginningTimeStamp.length > 0) {
                            ...
                            lineElement.dataset.start = endBefore;
                            karaElement.dataset.start = firstDecodeTime;
                        }
                        // set timing before 0
                        if(!beforeContent) {
                            if(firstDecodeTime < 3 && endBefore == 0) {
                                lineElement.dataset.start = 0;
                            }else{
                                if(firstDecodeTime - beforeTime > 3)
                                lineElement.dataset.start = firstDecodeTime - 3;
                            }
                        }
                        ...
                        if (endingTimeStamp.length > 0) {
                            ...
                            karaElement.dataset.end = this.decodeTimeStamp(endingTimeStamp[0]);
                        } else {
                            lineElement.dataset.end = Infinity;
                            karaElement.dataset.end = Infinity;
                            lineElementsWithoutEndingTime.push(lineElement);
                            lineElementsWithoutEndingTime.push(karaElement);
                        }
                        ...
                        if (!line) {
                            ...
                            if(posTwoSub != -1) {
                                karaElement.style.top = '0px';

                                let lineSub1Element = document.createElement('div');
                                let lineSub2Element = document.createElement('div');
                                lineSub1Element.className = 'sub1';
                                lineSub2Element.className = 'sub2';
                                let contentSub1 = contentLyric.substr(0, posTwoSub + 0);
                                let contentSub2 = contentLyric.substr(posTwoSub + 4, 9999)
                                lineSub1Element.innerHTML = contentSub1;
                                lineSub2Element.innerHTML = contentSub2;

                                karaElement.innerHTML = contentSub1;
                                lineSub1Element.appendChild(karaElement);

                                lineElement.appendChild(lineSub1Element);
                                lineElement.appendChild(lineSub2Element);
                                $('#csnplayer').css('height', '110px');
                                $('#lyrics').addClass('two_sub');
                            }else{
                                karaElement.innerHTML = contentLyric;
                                lineElement.innerHTML = contentLyric;
                                lineElement.appendChild(karaElement);
                            }

                        }
                    }else{
                        // set time to min near
                    }
                    beforeTime = firstDecodeTime;
                    beforeContent = contentLyric;
                }
            ...
            synchronizer() {
                ...
                this.lineElements.forEach(element => {
                    ...
                    if (time >= element.dataset.start && time <= element.dataset.end) {
                        ...
                        let karaStart = kara.attr("data-start");
                        let karaEnd = kara.attr("data-end");
                        let totalPlay = (karaEnd - time);

                        if(time >= (karaStart - 0.5)) {
                            let widthCurrent = $(kara).width();
                            $(kara).animate({ width: 100 + "%" }, (totalPlay * 1000));
                        }
                        this.jw_player.on('pause', function () {
                            let widthCurrent = $(kara).width();
                            $(kara).stop( true, true );
                            // $(kara).css('width', ((time - karaStart) * 100)/(karaEnd - karaStart) + "%");
                            $(kara).css('width', widthCurrent);
                        })
                        ...
                        if (element.classList.contains('active')) {
                            ...
                            let notEleActive = $(element).find('.kara-csn');
                            notEleActive.stop( true, true );
                            notEleActive.css('width', '0%');
                        }
                    }
                });
