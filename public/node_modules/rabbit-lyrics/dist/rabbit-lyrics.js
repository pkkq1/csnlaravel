    ...
    parseLyrics() {
        ...
        for (let i = 0; i < lyrics.length; i++) {
            ...
            if(timeLength > 1) {
                if(timeStamps[0] > timeStamps[1]) {
                    let strContent = line.replace(/\[\d+:\d+\.\d+\]/g, '');
                    if(timeLength == 2) {
                        lyricRaw.push(timeStamps[0] + strContent);
                        lyricRaw.push(timeStamps[1] + strContent);
                    }else {
                        timeStamps.forEach(function(item, key, object) {
                            let compareLyric = item;
                            for(let ii = (object.length - 1); ii > 0; ii--) {
                                if(item < object[ii]) {
                                    compareLyric = compareLyric + object[ii];
                                    object.splice(ii, 1);
                                    break;
                                }
                            }
                            delete timeStamps[key];
                            lyricRaw.push(compareLyric + strContent);
                        });
                    }
                }else{
                    lyricRaw.push(line);
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (beginningTimeStamp.length > 0) {
                let startTime = this.decodeTimeStamp(beginningTimeStamp[0]);
                lineElement.dataset.start = this.decodeTimeStamp(beginningTimeStamp[0]);
                karaElement.dataset.start = this.decodeTimeStamp(timeStamps[0]);
                if(timeStamps.length > 1) {
                    karaElement.dataset.start = this.decodeTimeStamp(timeStamps[1]);
                    if((this.decodeTimeStamp(timeStamps[1]) - this.decodeTimeStamp(timeStamps[0])) > 3) {
                        lineElement.dataset.start = this.decodeTimeStamp(timeStamps[1]) - 3;
                    }
                }
            } else {
